...
export class UtilsService {
  ...
  // todo:: add unit test
  convertToMomentArray(value: CalendarValue, format?: string, allowMultiSelect: boolean = false, locale: string = 'fa'): Moment[] {
    switch (this.getInputType(value, allowMultiSelect)) {
      case (ECalendarValue.String):
        return value ? [moment(<string>value, format, true).locale(locale)] : [];
      // case (ECalendarValue.StringArr):
      //   const res = (<string[]>value).filter(r => r.).map(v => v ? moment(v, format, true)?.locale(locale) : null).filter(Boolean)
      //   if(res)
  ...
  onlyTime(m?: Moment): Moment {
    if (m && moment.isMoment(m)) {
      return moment(m.format('HH:mm:ss'), 'HH:mm:ss');
    } else {
      return moment(moment().format('HH:mm:ss'), 'HH:mm:ss');
      ;
    }
    // return m && moment.isMoment(m)
  }
  ...
  createValidator({ minDate, maxDate, minTime, maxTime }: DateLimits,
    ...
    return (inputVal: CalendarValue) => {
      ...
      value = this.convertToMomentArray(inputVal, format, true, locale).filter(r => r != null);
  ...
  getHeaderLabel(config: IDayCalendarConfigInternal, year: Moment, type: string): string[] {
    ...
    let labels: string[] = [];
    for (let i = 0; i < (config.months ?? 1); i++) {
      let _year = (config.months ?? 1) > 1 ? year.clone().add(i, 'month') : year;
